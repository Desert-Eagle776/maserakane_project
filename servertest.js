...
function validateToken(req, res, next) {
  ...
  jwt.verify(token, JWT_SECRET, (err, user) => {
    ...
    if (!user.playerId) {
      return res
        .status(403)
        .json({ message: "Invalid token: playerId not found" });
    }
  ...
  db.serialize(() => {
    ...
    db.run(
      ...
      (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ message: "Database error: Failed to create player." });
        }
    ...
    db.run(
      ...
      (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ message: "Database error: Failed to store token." });
        }
    ...
    db.run("COMMIT", (err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: "Database error: Failed to commit transaction." });
      }
...
app.post("/api/generateToken", (req, res) => {
  ...
  db.get("SELECT id FROM players WHERE id = ?", [playerId], (err, player) => {
    if (err) {
      console.error(
        "Database error while checking player existence:",
        err.message
      );
    ...
    db.get(
      "SELECT token, expires_at FROM tokens WHERE player_id = ?",
      [playerId],
      (err, tokenRow) => {
        if (err) {
        ...
        const newToken = jwt.sign({ playerId }, JWT_SECRET, {
          expiresIn: "1h",
        });
        const expiresAt = new Date(
          now.getTime() + 60 * 60 * 1000
        ).toISOString(); // 1 heure d'expiration
        ...
        db.run(
          ...
          (err) => {
            if (err) {
              console.error(
                "Database error while inserting/updating token:",
                err.message
              );
              return res
                .status(500)
...
app.get("/api/inventory/:wallet", (req, res) => {
  ...
  db.get(
    "SELECT inventory FROM players WHERE wallet = ?",
    [wallet],
    (err, row) => {
      if (err) {
...
app.post("/api/lumberjack/chop", validateToken, async (req, res) => {
  ...
  try {
    ...
    if (mapsHashmap[mapId]) {
      console.log(
        "Available positions in map:",
        Object.keys(mapsHashmap[mapId])
      );
    } else {
    ...
    db.get(
      "SELECT stuff, inventory, profession_xp FROM players WHERE id = ?",
      [playerId],
      (err, row) => {
        if (err || !row) {
          console.error(
            "Error fetching player data from database or player not found:",
            err?.message
          );
        ...
        professionXP.lumberjack =
          (professionXP.lumberjack || 0) + axeConfig.xpGained;
        ...
        db.run(
          ...
          (err) => {
            if (err) {
              console.error(
                "Failed to update player data in database:",
                err.message
              );
              return res
                .status(500)
                .json({ message: "Failed to update player data." });
            }
    ...
    db.get(
      ...
      (err, row) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ message: "Failed to fetch player data." });
        }
        if (!row) {
        ...
        const equippedRod =
          stuff.prof_item_1 === equippedRodKey ||
        ...
        console.log(
          "Worm bait deducted. Remaining worm bait:",
          inventory.worm_bait
        );
...
app.post("/craft/blacksmith", validateToken, async (req, res) => {
  ...
  if (!type || !material || !rarity) {
    return res
      .status(400)
      .json({ error: "Missing required fields: type, material, or rarity" });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Server error: Unable to read crafting data" });
  }

  if (
    !craftingData[type] ||
    !craftingData[type][material] ||
    !craftingData[type][material][rarity]
  ) {
    return res
      .status(400)
      .json({ error: "Invalid crafting type, material, or rarity" });
  }

  const craft = craftingData[type][material][rarity];

  db.get(
    "SELECT profession_xp, inventory FROM players WHERE id = ?",
    [playerId],
    (err, row) => {
      if (err) {
      ...
      for (const requiredMaterial of craft.materials) {
        const playerMaterial = inventory.find(
          (m) => m.name === requiredMaterial.name
        );
        if (
          !playerMaterial ||
          playerMaterial.quantity < requiredMaterial.quantity
        ) {
          return res.status(400).json({
      ...
      const updatedInventory = inventory
        .map((item) => {
          const requiredMaterial = craft.materials.find(
            (m) => m.name === item.name
          );
          if (requiredMaterial) {
            return {
              ...item,
              quantity: item.quantity - requiredMaterial.quantity,
            };
          }
          return item;
        })
      ...
      db.run(
        ...
        (err) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ error: "Failed to update inventory and XP" });
          }
...
app.post("/craft/armorer", validateToken, async (req, res) => {
  ...
  if (!type || !material) {
    return res
      .status(400)
      .json({ error: "Missing required fields: type or material" });
  }
  ...
  if (!craft) {
    return res
      .status(400)
      .json({ error: "Invalid crafting rarity or base item not found" });
  }
    ...
    (err, row) => {
      ...
      try {
        ...
        if (!Array.isArray(inventory)) {
          console.error(
            "Inventory data is not an array, resetting to empty array."
          );
          inventory = [];
        }
      } catch (error) {
      ...
      if (isSuccess) {
        ...
        const craftedItemName = rarity
          ? `${rarity} ${material} ${type}`
          : `${material} ${type}`; // Exemple : "Iron Chestplate" ou "Rare Iron Chestplate"
        ...
        db.run(
          ...
          [
            JSON.stringify(updatedInventory),
            JSON.stringify({ ...professionXp, armorer: playerXp + craft.xp }),
            playerId,
          ],
          (err) => {
            if (err) {
          ...
          (err) => {
            if (err) {
              ...
              return res
                .status(500)
                .json({
                  error: "Failed to update inventory after failed craft",
                });
            }
...
app.post("/craft/basic", validateToken, async (req, res) => {
  try {
    ...
    if (!playerId)
      return res
        .status(401)
        .json({ error: "Unauthorized: Player ID not found" });
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Server error: Unable to read basic crafting data" });
    }

    const craft = basicCraftData[item];
    if (!craft)
      return res.status(400).json({ error: "Invalid basic crafting item" });
    ...
    db.get(
      ...
      (err, row) => {
        ...
        try {
          ...
          return res
            .status(500)
            .json({ error: "Server error: Inventory parsing failed" });
        }
        ...
        for (const material of craft.materials) {
          if (
            ...
            inventory[material.name] < material.quantity
          ) {
            console.log("Insufficient material:", material);
            return res
              .status(400)
              .json({ error: `Insufficient materials for ${material.name}` });
          }
        }
        ...
        if (success) {
          ...
          // Update XP
          professionXp[craft.jobs] =
            (professionXp[craft.jobs] || 0) + craft.xpGained;
        } else {
          craft.materials.forEach(({ name, quantity }) => {
            inventory[name] -= quantity;
            if (inventory[name] <= 0) delete inventory[name];
